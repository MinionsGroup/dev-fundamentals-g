     ...
     public void showAll () {
        System.out.println("  --*-*-*-*-*-*-*-*-*-*-* PLAYERS-*-*-*-*-*-*-*-*-*-*--*- ");
            for (int i = 0; i < carCount; i++) {
                System.out.println(i + 1 + ".- " + myCars[i].nameplayer + "        " + myCars[i].performace + " kilometers/lts       " + myCars[i].gasTank+ " Liters of gasoline in the tank");
      ...
      void  distancePerCar  () {
        ...
        System.out.println( "*-*-*-*-*- Final distance related with performance and gas in the tank are: ");
         ...
         System.out.println( i+1+".- The distance traveled by the player " +myCars[i].nameplayer+" is:  "+distanceGas[i]+" Kilometers");
